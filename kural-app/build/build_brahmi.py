...
"""build_brahmi.py — the 1,330 couplets in தமிழி (Tamil-Brahmi, Unicode Brahmi block) → data/brahmi.json
  ...
  py build/build_brahmi.py             # data/brahmi.json + build/brahmi_review.txt + build/Tirukkural-Tamil-Brahmi.txt, with checks
  py build/build_brahmi.py --e-virama  # the older convention, written ONLY to build/Tirukkural-Tamil-Brahmi-compat.txt;
                                       # data/brahmi.json is untouched
...
convention of older converters. Measured in Chromium with both Noto Sans Brahmi and Segoe UI Historic: "vowel sign +
virama" is rejected by the shaper and drawn with a dotted circle, so that convention is kept only for interchange
with tools that expect it, never for display.

Fonts: the Old Tamil code points need a font built for Unicode 11 or later — Noto Sans Brahmi v2.004 (bundled by
the app) or Windows' Segoe UI Historic. Older copies of Noto Sans Brahmi installed system-wide lack them and show
boxes; build/Tirukkural-Tamil-Brahmi.pdf is rendered with the bundled font so it reads the same everywhere.
...
SHORT = "" if OLD else B("VIRAMA")   # what turns a long e/o into the short one in the compatibility convention
...
DIGITS = {str(d): B("DIGIT " + n) for d, n in enumerate(DIGIT_NAMES)}
...
def to_brahmi(s):
    ...
    for ch in ud.normalize("NFC", s):
        ...
        elif ch in " ?" or ch == NL: out.append(ch)
        elif ch == ".": pass
        else: raise ValueError(f"no Brahmi for {ch!r} U+{ord(ch):04X} {ud.name(ch, '?')}")
    return "".join(out)

def to_tamil(b):
    # longest-match reverse: two-code-point short vowels before the plain letter they contain
...
def write_text(rows, path, convention):
    """The whole book as plain text, chapter by chapter, each couplet numbered in Brahmi digits."""
    m = json.load(open(APP / "data/meta.json", encoding="utf-8"))
    out = ["திருக்குறள் — தமிழி (தமிழ்ப் பிராமி) · Tirukkural in Tamil-Brahmi (Unicode Brahmi block)",
           "இன்றைய எழுத்துக்கூட்டலை எழுத்துக்கு எழுத்து மாற்றியது; கல்வெட்டு மறுவாக்கம் அன்று. குறள் எண்கள் பிராமி இலக்கங்களில்.",
           "Central Institute of Classical Tamil · build/build_brahmi.py · " + convention, ""]
    for ch in m["chapters"]:
        out.append(f"{NL}{ch['adhigaram']}. {ch['name']} · {ch['nameEn']}{NL}")
        for n in range(ch["start"], ch["end"] + 1):
            l1, l2 = rows[n]
            out.append(f"{l1}{NL}{l2} ({brahmi_number(n)}){NL}")
...
def main():
    ...
    for c in range(1, 134):
        for k in json.load(open(APP / f"data/ch/{c:03d}.json", encoding="utf-8"))["kurals"]:
            ...
            review.append(f"{k['n']}{NL}{k['l1']}{NL}{k['l2']}{NL}{l1}{NL}{l2}{NL}")
    assert not bad, f"round trip failed for {bad[:10]}"
    if not OLD:   # the compatibility text only — never the app's data
        write_text(rows, APP / "build/Tirukkural-Tamil-Brahmi-compat.txt", "e-plus-virama (compatibility: pre-Unicode-11 fonts)")
        print(f"{len(rows)} couplets → build/Tirukkural-Tamil-Brahmi-compat.txt (data/brahmi.json untouched)")
        return
    meta = {"script": "Tamil-Brahmi (தமிழி), Unicode Brahmi block", "convention": "unicode11-old-tamil",
    ...
    (APP / "build/brahmi_review.txt").write_text(NL.join(review), encoding="utf-8")
    write_text(rows, APP / "build/Tirukkural-Tamil-Brahmi.txt", meta["convention"])
