...
async function viewVerify(q0) {
  ...
  const run = async () => {
    ...
    box.innerHTML = `<div class="card">
      <div class="kural-head"><span class="n">${t('kural')} ${best.n}</span><a class="ch" href="#/ch/${cm.adhigaram}">${esc(cm.name)} · ${esc(enN(cm.name, cm.nameEn))}</a><span class="pill">${Math.round(best.score * 100)}% ${t('verifyScore')}</span></div>
